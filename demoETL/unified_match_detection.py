...
class UnifiedMatchDetector:
    ...
    def poll_faceit(self, player_ids: List[str] = None) -> List[Dict]:
        """
        Poll Faceit for new matches.

        Args:
            player_ids: List of Faceit player IDs (optional, reads from DB if None)
        ...
        try:
            # Pass None to read player IDs from database
    ...
    def poll_all_sources(
        ...
        Args:
            faceit_players: List of Faceit player IDs (optional, reads from DB if None)
        ...
        # Poll Faceit (pass None to read player IDs from database)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source', type=str, help='Match source (steam, faceit)')
    ...
    try:
        if args.summary:
            ...
            # Parse player IDs (if not provided, Faceit will read from database)
            faceit_players = args.faceit_players.split(',') if args.faceit_players else None
            ...
            # Poll all sources (Faceit reads player IDs from database)
            new_matches = detector.poll_all_sources()
