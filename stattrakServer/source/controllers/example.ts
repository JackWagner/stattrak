...
import {
  ...
  PlayerDeathEvent,
  RoundEndEvent,
} from "../common/example/example.interface";
...
const getKillsPerRound = async (res: Response) => {
  ...
  const validKills = killsNoWarmup.filter(
    (kill) => kill.attacker_team_name != kill.user_team_name
  );
  const latestRoundWithKill = Math.max(
    ...kills.map((kill) => kill.total_rounds_played)
  );

  const killsPerPlayer: Map<string, number> = new Map();
  for (let round = 0; round <= latestRoundWithKill; round++) {
    const killsThisRound = validKills.filter(
      (kill) => kill.total_rounds_played == round
    );
    killsThisRound.forEach((kill) => {
      ...
      else killsPerPlayer.set(attackerName, 1);
    });
  }
...
const killsPerRound = async (req: Request, res: Response) => {
  ...
  if (Object.keys(req.body).length === 0) {
    return await getKillsPerRound(res);
  }
